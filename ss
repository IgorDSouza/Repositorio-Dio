// Exemplo 7 -Módulo Relé 1 
Canalint buttonPin = 3; // pino D3 para botao
int ledPin = 13; // LED na placa Arduinoint 
IN1 = 2; // pino D2 para controle do rele
...
void setup(){pinMode(ledPin, OUTPUT); // pino do LED = saida
...
digitalWrite(IN1, ledState); // desliga o rele
    ...
    if (reading != lastButtonState) // se o estado for diferente do anterior{
        ...
        }if ((millis() -lastDebounceTime) > debounceDelay){// se o tempo do botao pressionado for maior do que debounce
